...
from models.DDOIN import DDODDOIN_motion_correction
...
#load model
    ...
    if not os.path.exists("results/ddo_ori"):
        os.makedirs("results/ddo_ori")
    ...
    for scenario in scenarioius:
        ...
        for idx_test in trange(batch_test):
            ...
            refined_image, _ = DDOmoco_motion_correction(
        ...
        fig.savefig("results/ddo_ori/immoco_" + scenario + ".png", bbox_inches="tight")
        plt.close(fig)

        metrics_all.append(metrics)

    # save metrics
    torch.save(metrics_all, "results/ddo_ori/immoco_metrics.pth")
    ...
    metrics_all = torch.load("results/ddo_ori/immoco_metrics.pth")
    ...
    with open("results/ddo_ori_matrix.tex", "w") as f:
