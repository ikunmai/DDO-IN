...
# initialize wandb
wandb.init(project="DDOIN", group="KMotion_recognize", name="KMotion_recognize", mode="online")
...
for i in trange(EPX):
    ...
    with torch.no_grad():
        ...
        calassification_metrics = metrics_classification((output_test.sigmoid() > 0.5).float(),
                                                         mask_test.squeeze().float())

        wandb.log({"dice_score": dice_score, "epoch": i})
        wandb.log({"iou": iou, "epoch": i})
        # log pixelwise accuracy
        wandb.log(
            {"pixelwise_accuracy": torch.sum((output_test.sigmoid() > 0.5) == mask_test.squeeze().float()) / (H * W),
             "epoch": i})
